Feature: To validate the create booking API endpoint
  ...
  Scenario: To verify the create booking endpoint with valid payload and authentication.
    Given User creates booking with:
      ...
      | phone       | 12345678901          |
    When User makes a "POST" action to the "createBookingEndpoint" via "authToken"
  ...
  Scenario Outline: To verify the negative scenarios of create booking end point by passing incorrect payload
    ...
    Examples:
      ...
      #LastName Validation
      # Note: Test cases (lines 54–55) may fail due different Lastname upper limit.
      ...
      # Note: Test cases (lines 62) may fail due to API not configured to return
      ...
      # Note: Test cases (lines 69) may fail due to API not configured to return
            # standard error message for missing mandatory fields.
      | RANDOM_1_10  | John                | Doe                 | true        | +1      | +2       |                      | 12345678901            | Email should not be blank           |
      #Dates Validation
      # Note: Test cases (lines 54–55) may fail due to checkout and checkin date not properly handled.
