Feature: To validate the create booking API endpoint
  ...
  Scenario: To verify the create booking endpoint with valid payload and authentication.
    ...
    #Note: Validations from lines 28–29 will fail since API is not designed to have email, phone as part of response.Scenario:
  ...
  Scenario Outline: To verify the negative scenarios of create booking end point by passing incorrect payload
    ...
    Examples:
      ...
      #LastName Validation
      # Note: Test cases (lines 62–64) may fail due different Lastname upper limit.
      ...
      # Note: Test cases (lines 77) may fail due to API not configured to return
            # standard error message for missing mandatory fields.
      | RANDOM_1_10  | John                | Doe                 | true        | +1      | +2       |                      | 12345678901            | Email should not be blank           |
      #Dates Validation
      # Note: Test cases (lines 82–87) may fail due to checkout and checkin date not properly handled.
