Feature: To validate the get booking API endpoint
  ...
  @getBooking @positive
  Scenario: To verify the get booking endpoint with valid payload.
    ...
    #Note: Validations from lines 18-19 will fail since API is not designed to have email, phone as part of response.Scenario:
  ...
  Scenario Outline: To verify the get booking endpoint with invalid request specifications.
    ...
    Examples:
      ...
      #Note: Test cases (lines 39) may fail due to API not configured to return proper status code when payload is not passed.
